...
from datalib.code import Code
from datalib.example import Example
from datalib.parser_define_xml import ParserDefineXML
from define.dataset import Dataset
from define.variable import Variable
...
if __name__ == '__main__':
    # print(Code.markdown((("ts", "TSPARMCD"),("cv", "CVFATSCD"),)))
    # print(Code.get("ts","TSPARMCD"))
    rules = [
    ...
    for r in rules:
        ...
        code_desc = Code.markdown((("ts", "TSPARMCD"),))
        # print(dataset)
        expression = NormalizeAgents().run(r, dataset, code_desc).out_put()
        print(expression)

        #
        # example = Example(Path("datalib//example"))
        # answers = TestCaseAgents().debug(rule=expression,
        #                                  dataset=dataset,
        #                                  code_desc=code_desc,
        #                                  example=example.load([Dataset(domain="TS", variable=["TSPARMCD", "TSVAL"])]))
        # print(answers)
