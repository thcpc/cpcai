...
class BaseModel(Base):
    ...
    def as_dict(self, **renames):
        if not renames:
            return {c.name: self.covert(getattr(self, c.name)) for c in self.__table__.columns}
        else:
            return {renames.get(c.name, c.name): self.covert(getattr(self, c.name)) for c in self.__table__.columns}
